...
def render_navbar(activo):
    ...
    # Diccionario local ultrarrápido y aislado para la barra
    nav_txt = {
    ...
    with c1:
        if st.button(txt[0], use_container_width=True, type="primary" if activo == "lobby" else "secondary", key=f"btn_nav_1_{activo}"):
            st.session_state.estado = "lobby"; st.rerun()
    with c2:
        if st.button(txt[1], use_container_width=True, type="primary" if activo == "mundo" else "secondary", key=f"btn_nav_2_{activo}"):
            st.session_state.estado = "mundo"; st.rerun()
    with c3:
        if st.button(txt[2], use_container_width=True, type="primary" if activo == "cuartel" else "secondary", key=f"btn_nav_3_{activo}"):
            st.session_state.estado = "cuartel"; st.rerun()
    with c4:
        if st.button(txt[3], use_container_width=True, type="primary" if activo == "tienda" else "secondary", key=f"btn_nav_4_{activo}"):
            st.session_state.estado = "tienda"; st.rerun()
    with c5:
        if st.button(txt[4], use_container_width=True, type="primary" if activo == "gremio" else "secondary", key=f"btn_nav_5_{activo}"):
